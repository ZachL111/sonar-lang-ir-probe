...
`sonar-lang-ir-probe` is centered on compilers work in Julia. The technical target is: Create a Julia reference implementation for ir workflows, centered on visual model generation, layout fixtures, and stable geometry snapshots.
...
The score is deterministic. The current threshold is 153. Risk has a penalty of 5, latency has a penalty of 2, and weight contributes a bonus of 6.
